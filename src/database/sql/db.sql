...
BEGIN
    ...
    DECLARE cv0, cv1 VARBINARY(256) DEFAULT '';
    ...
    SET i = 1;
    WHILE i <= s1_len DO
        SET s1_char = SUBSTRING(s1, i, 1), cv0 = UNHEX(HEX(i)), j = 1;
        WHILE j <= s2_len DO
            SET c = j + 1;
...
DELIMITER $$
CREATE OR REPLACE FUNCTION NORMALIZE_TITLE(s TEXT)
RETURNS TEXT
DETERMINISTIC
BEGIN
    DECLARE normalized TEXT;
    SET normalized = CONVERT(CONVERT(s USING ascii) USING utf8mb4);
    SET normalized = LOWER(REGEXP_REPLACE(normalized, '[^a-zA-Z0-9 ]', ' '));
    RETURN normalized;
END$$
DELIMITER ;